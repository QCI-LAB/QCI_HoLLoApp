classdef QCI_Model_GPU < QCI_Model
    ...
    methods
        ...
        function reconstruction = GerchbergSaxton(obj, iterationCount, gaussianSigma)
            ...
            hologramsOriginal = gpuArray(obj.Holograms);
            kernels = gpuArray(obj.PropagationKernels);
            ...
            for iter = 1:iterationCount
                for idx = 1:(numberOfHolograms - 1)
                    reconstruction = propagateOptimized(inputField(:,:, idx), kernels(:,:,idx));
                    ...
                    inputField(:, :, idx+1) = propagateOptimized(reconstruction, kernels(:,:,idx+1), true);
                    inputField(:, :, idx+1) = amplitudeImages(:,:, idx+1).*inputField(:,:, idx+1)./abs(inputField(:, :, idx+1));
                end
                
                reconstruction = propagateOptimized(inputField(:,:,end), kernels(:,:,end));

                inputField(:, :, 1) = propagateOptimized(reconstruction, kernels(:,:,1), true);

                inputField(:, :, 1) = amplitudeImages(:, :, 1).*inputField(:, :, 1)./abs(inputField(:,:,1));
            end
                    
            % Final propagation to object plane
            reconstruction = propagateOptimized(inputField(:,:, 1), kernels(:,:, 1));
